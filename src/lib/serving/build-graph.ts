...
const library = JSON.parse(
  fs.readFileSync(`${dir}/library-items.json`, "utf-8")
);

const content = JSON.parse(
  fs.readFileSync(`${dir}/content-items.json`, "utf-8")
);

const tagIndex = JSON.parse(
  fs.readFileSync(`${dir}/tag-index.json`, "utf-8")
);

const amarakoshaVargas = JSON.parse(
  fs.readFileSync(`${dir}/amarakosha-varga-map.json`, "utf-8")
);
...
amarakoshaVargas.forEach((varga: any) => {
  const vargaId = makeId("concept", varga.slug);
  ...
  addNode({
    ...
    meta: {
      source: "derived",
      // Store stripped IAST slug and Devanagari for display
      route: undefined,
      image: undefined,
      author: undefined,
      // @ts-ignore — extended meta for Amarakosha vargas
      iast: varga.iast,
      devanagari: varga.devanagari,
      wordCount: varga.wordCount,
      amarakosha: true
    },
    classification: {
      tags: [],
  ...
  // Link varga → our existing concept nodes
  varga.conceptSlugs.forEach((targetSlug: string) => {
    const targetId = makeId("concept", targetSlug);
    if (nodeMap.has(targetId)) {
      addEdge(vargaId, targetId, "RELATED_TO", { source: "manual" });
      // Also add reverse edge so concepts link back to their varga
      addEdge(targetId, vargaId, "RELATED_TO", { source: "manual" });
    }
  });
});
...
fs.writeFileSync(`${outDir}/nodes.json`, JSON.stringify(finalNodes, null, 2));
