  ...
  export async function allArch() {
	...
	const eachfiled = await Promise.all(
	  filed.map(async ([path, resolver]) => {
		...
		return {
		  meta: metadata,
		  linkpath: `/library/journal-archaeology/${slug}`
		};
	  })
	)
	const validPosts = eachfiled.filter((post): post is NonNullable<typeof post> => post !== null);
...
export async function pramanaCards(){
	const posts = import.meta.glob('/src/routes/designbodha/pramana-library/*.md')
	const allfiles = { ...posts };
	const filed = Object.entries(allfiles)
	const eachfiled = await Promise.all(
		filed.map(async ([path,resolver]) => {
...
export async function reading1000(){
	const posts = import.meta.glob('/src/routes/library/bharata1000/*.md')
	const allfiles = { ...posts };
	const filed = Object.entries(allfiles)
	const eachfiled = await Promise.all(
		filed.map(async ([path,resolver]) => {
	...
	return eachfiled.sort((a, b) => a.meta.id - b.meta.id);
}