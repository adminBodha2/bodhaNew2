<script lang="ts">
  ...
  let books:any
  $metaTitle = 'Bodha - Library | General'
  $metaDescription = 'General readings at Bodha library.'
  $metaUrl = 'https://www.bodharesearch.in/library/general'
  $metaImage = '/images/bodhacover.png'

  onMount(() => {
    (async() => {
      books = await selectedOpenLibrary('general');
    })();
  })
