					...
					<div class="grid lg:grid-cols-3 xl:grid-cols-4 b-main radius" id="node-grid">
						{#each group.items as item, j (item.node.id)}
							<div class="number box">
								<a class="blank box p16 lg:p32 rgap8" href={item.href} target={item.isExternal ? '_blank' : undefined} rel={item.isExternal ? 'noreferrer' : undefined} in:slide|global={{ delay: j * 30, easing: quintOut }} out:slide={{ easing: quartInOut }}>
									<p class="body-text w600 tight">{item.node.title}</p>
								...
								<div class="row cgap4 rgap4 wrap ycenter self-bottom bordertop pleft16 lg:pleft32 ptop16 pbot16 tags">
									{#each item.node.tags as tag}
										<a class="cite hollow tt-u" href="/concepts/{tag}">{tag.replaceAll('-', ' ')}</a>
									{/each}
								</div>
							</div>
						{/each}
