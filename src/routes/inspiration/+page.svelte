...
<Container>
	...
	<section class="wrapper-std min100vh growingline">
		<Title text="Schools of Thought" />
		{#if schools.length > 0}
			<Blur targetSelector=".blur-item" duration={300} end="bottom: 50%">
				<div class="grid grid-cols-2 lg:grid-cols-4 gap16">
					{#each schools as item, i}
						<a class="inspiration-card blank blur-item" href={item.linkpath}>
							<img class="card-image" src={item.meta.image} alt={item.meta.title} />
							<div class="card-overlay">
								<p class="txt-xl white w500">{item.meta.title}</p>
							</div>
						</a>
					{/each}
				</div>
	...
	<section class="wrapper-std growingline alternate">
		...
		{#if thinkers.length > 0}
			<Blur targetSelector=".blur-item2">
				<div class="grid grid-cols-2 lg:grid-cols-5 gap16" bind:this={ref3}>
					{#each thinkers as item, i}
						<a class="inspiration-card blank blur-item2" href={item.linkpath}>
							<img class="card-image" src={item.meta.image} alt={item.meta.title} />
							<div class="card-overlay">
