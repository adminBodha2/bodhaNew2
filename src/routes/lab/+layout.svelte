        ...
        <nav class="lab-nav">
          {#each Object.entries(grouped) as [category, items]}
            <div class="lab-nav-group">
              <p class="lab-nav-group-label">{category}</p>
              {#each items as item}
                {@const isActive = $page.url.pathname === item.linkpath}
                <a class="lab-nav-item" href={item.linkpath} class:active={isActive}>
                  <span class="lab-nav-title">{item.meta.title}</span>
                </a>
              {/each}
            </div>
          {/each}
